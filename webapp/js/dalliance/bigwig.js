...
BigWigView.prototype.readWigDataById = function(chr, min, max, callback) {
    ...
    var cirFobRecur = function(offset, level) {
        ...
        var spans = new Range(offset[0],offset[offset.length-1] + maxCirBlockSpan);

        var fetchRanges = spans.ranges();
        cirFobStartFetch(offset, fetchRanges[0], level);
    }

    var cirFobStartFetch = function(offset, fr, level, attempts) {
        //console.log(fr.min(),fr.max());
        ...
        if (isLeaf != 0) {
            ...
            for (var i = 0; i < cnt; ++i) {
                ...
                if ((startChrom < chr || (startChrom == chr && startBase <= max)) &&
                    (endChrom   > chr || (endChrom == chr && endBase >= min)))
                {
                    //console.log(startChrom, endChrom, startBase, endBase);
                    recurOffsets.push(blockOffset);
                }
                //console.log("overlaps ",recurOffsets.length);
                offset += 24;
            }
...
BigWigView.prototype.createFeature = function(chr, fmin, fmax, opts) {
    if (!opts) {
        opts = {};
    }
    ...
    var f = {};
    f._chromId = chr;
    f.segment = this.bwg.idsToChroms[chr];
    f.min = fmin;
    f.max = fmax;
    f.type = this.bwg.type;
    ...
    if (blocksToFetch.length == 0) {
        ...
        var tramp = function() {
            if (blocksToFetch.length == 0) {
                ...
                if (block.data) {
                    thisB.parseFeatures(block.data, this.createFeature, filter);
                    ...
                    //console.log("blocktotal ",fetchStart,fetchSize);
    ...
    if (this.isSummary) {
        ...
        for (var i = 0; i < itemCount; ++i) {
            ...
            if (filter(chromId, start + 1, end)) {
                ...
                this.createFeature(chromId, start + 1, end, summaryOpts);
            }
        }
        ...
        var blockType = ba[20];
        var itemCount = sa[11];
        //console.log(chromId, blockType, itemCount);
        //console.log(itemCount, blockStart, blockEnd, chromId);
        ...
        if (blockType == BIG_WIG_TYPE_FSTEP) {
            ...
            for (var i = 0; i < itemCount; ++i) {
                ...
                if (filter(chromId, fmin, fmax)){
                    this.createFeature(chromId, fmin, fmax, {score: score});
                    //values.push([start, end, score]);
                }
            }
            ...
            for (var i = 0; i < itemCount; ++i) {
                ...
                if (filter(chromId, start, end)){
                    //values.push([start, end, score]);
                    this.createFeature(chromId, start, end, {score: score});
                }
            }
        ...
        while (offset < ba.length) {
            ...
            if (filter(chromId, start + 1, end, bedColumns)) {
                if (dfc < 12) {
                    this.createFeature(chromId, start + 1, end, featureOpts);
                } else {
                    ...
                    for (var s = 0; s < tsList.length; ++s) {
                        var ts = tsList[s];
                        this.createFeature(chromId, ts.min() + 1, ts.max(), featureOpts);
                    }

                    if (thickEnd > thickStart) {
                        //console.log("thickEnd > thickStart");
                        ...
                        if (tl) {
                            //console.log(features.length,start, end, spans, codingRegion, tl);
...
BigWig.prototype.getUnzoomedView = function() {
    ...
    if (!this.unzoomedView) {
        ...
        //console.log("RTree at "+this.unzoomedIndexOffset);
...
function makeBwg(data, name, remote, callback) {
    var bwg = new BigWig();

    if(remote) data = new URLFetchable(data);
    else data = new BlobFetchable(data);
